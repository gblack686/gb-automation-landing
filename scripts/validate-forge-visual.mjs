...
import {newJob,transition,QUOTES,digest,assetKey} from '../amplify/functions/forge-visual/domain.mjs';
...
try{
 await page.exposeFunction('visualRequest',async(input,write)=>{
  ...
  if(input.action==='list')return {jobs:[...jobs.values()],operator:true,quotes:QUOTES,active_id:activated};
  if(input.action==='create'){const j=newJob(input.id,input.brief,actor,now);jobs.set(j.id,j);return j;}
  const j=jobs.get(input.id);assert(j);
  if(input.action==='get')return j;
  ...
  jobs.set(next.id,next);return next;
 });
 ...
 for(const [stage,label] of [['portrait','portrait'],['character','full character'],['master','3d master'],['web','web model']]){
  ...
  assert.equal(actions.filter(a=>a.action==='start').length,['portrait','character','master','web'].indexOf(stage));
 ...
 await page.getByRole('button',{name:'Use approved character in Forge',exact:true}).click();await page.getByText('Approved portrait and 3D model are now used in Forge.').waitFor();assert(activated);
 ...
 assert.deepEqual(errors,[]);assert.equal(actions.filter(a=>a.action==='start').length,4);
 const receipt={ok:true,checks:['Saving does not generate','Each paid stage requires separate bound confirmation','Rendered output required before review','Actual GLTFLoader front/side/back controls','Web gate before adoption','Mobile fit','No JavaScript errors'],providers:'synthetic; no charges'};
 await writeFile(out+'/visual-browser.json',JSON.stringify(receipt,null,2));console.log(JSON.stringify(receipt));
}catch(error){console.error(JSON.stringify({error:String(error),errors,actions:actions.map(a=>a.action)}));throw error;}finally{await browser.close();}
