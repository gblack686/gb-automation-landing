...
export const STAGES = ['portrait','character','master','web'];
export const QUOTES = Object.freeze({portrait:'flare-low-portrait-v1',character:'flare-low-character-v1',master:'meshy6-4k-30-v1',web:'meshy-remesh-5-v1'});
...
export const assetKey = (job,role) => `${ROOT}/runs/${job.id}/${role}.${['master','web','remesh'].includes(role)?'glb':role==='manifest'?'json':role==='card'?'jpg':'png'}`;
...
export function requestFor(event,issuer) {
 ...
 if(!object(input)||JSON.stringify(input).length>16000||Object.keys(input).some(k=>!['action','id','revision','stage','sha256','quote','brief','decision'].includes(k)))fail('invalid_request');
 if(!(write?['create','start','review','adopt','resume']:['list','get','asset','active']).includes(input.action))fail('invalid_request');
...
export function newJob(id,brief,actor,now) {
 ...
 return {schema:'forge-visual-job.v1',id,tenant_id:TENANT,expert_id:EXPERT,config_sha256:CONFIG_SHA,revision:0,created_at:now,updated_at:now,created_by:actor,
...
export function assertJob(job,id=job?.id){
 ...
 for(const [role,a] of Object.entries(job.assets||{}))if(!['card','portrait','character','master','remesh','web'].includes(role)||a.key!==assetKey(job,role)||!/^[a-f0-9]{64}$/.test(a.sha256))fail('artifact_routing_failed');
 return job;
}
export const inputHash = (job,stage) => stage==='portrait'?job.brief_sha256:job.assets[{character:'portrait',master:'character',web:'master'}[stage]]?.sha256;
export function transition(job,input,actor,now) {
 ...
 if(input.action==='start') {
  if(!STAGES.includes(stage)||stage!==job.stage||job.status!=='draft'||current)fail('stage_not_ready');
  ...
  else {current.status='approved'; const following=STAGES[STAGES.indexOf(stage)+1];next.stage=following||stage;next.status=following?'draft':'ready';}
