...
import {frameFor,cardLayout} from '../../amplify/functions/forge-visual/card-frame.mjs';
...
export default function ForgeVisualStudio({brief,onClose,onAdopt}){
 ...
 const localCard=job?.stage==='agent_card'&&job.card_frame_version===1;
  let layoutError='';if(job?.card_frame_version===1&&!frameFor(job.brief.card))layoutError='This printing needs a verified frame map before generation.';
  else if(localCard&&cardText){try{cardLayout(job.brief.card,cardText,job.assets.card?.printing);}catch(e){layoutError=e.message;}}
