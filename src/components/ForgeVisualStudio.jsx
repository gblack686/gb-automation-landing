...
export default function ForgeVisualStudio({brief,onClose,onAdopt}){
 ...
 const [jobs,setJobs]=useState([]),[job,setJob]=useState(null),[operator,setOperator]=useState(false),[quotes,setQuotes]=useState({}),[busy,setBusy]=useState(false),[error,setError]=useState(''),[confirm,setConfirm]=useState(false),[view,setView]=useState('portrait'),[preview,setPreview]=useState(null),[viewed,setViewed]=useState(''),[loading,setLoading]=useState(false),[notice,setNotice]=useState(''),[cardText,setCardText]=useState(null);
 ...
 async function act(input){setBusy(true);setError('');try{const result=await visualRequest(input,true);if(input.action==='adopt'){setNotice('Approved visuals are now used in Forge.');onAdopt();}else{setJob(result);setJobs(list=>[result,...list.filter(x=>x.id!==result.id)]);}setConfirm(false);}catch(e){setError(e.message);}finally{setBusy(false);}}
 ...
 return <dialog ref={dialog} className="forge-visual-dialog" aria-labelledby="visual-title" onCancel={onClose}>
  ...
  <div className="visual-layout"><aside>
   ...
   <ol className="visual-stages">{stages.filter(([key])=>key!=='agent_card'||!job||job.pipeline_version===2).map(([key,icon,label])=><li key={key}><button className={view===key?'selected':''} disabled={!job?.assets[key]} onClick={()=>setView(key)}><span>{icon}</span><span>{label}<small>{job?.stages[key]?.status?.replaceAll('_',' ')||'Pending'}</small></span>{job?.stages[key]?.review?.decision==='approve'?'✓':''}</button></li>)}</ol>
   {job?.assets.source_card&&<button onClick={()=>setView('source_card')}>▣ Compare full source card</button>}
   ...
   {job&&<details className="visual-packet" open><summary>Output packet</summary><ul>{[['source_card','Full reference card'],['card','Reference art only'],['portrait','Agent portrait'],['agent_card','Full agent card'],['character','Full character · 3D input'],['master','Textured master GLB'],['web','Optimized web GLB']].filter(([key])=>job.pipeline_version===2||!['source_card','agent_card'].includes(key)).map(([key,label])=><li key={key}><span>{job.assets[key]?'✓':'○'}</span> {label}</li>)}</ul><small>Includes card text, source metadata, brief, approvals, costs, hashes and manifest.</small><button disabled={busy||job.status!=='ready'} onClick={async()=>{setBusy(true);setError('');try{await downloadVisualPacket(job.id,setNotice);setNotice('Complete approved packet downloaded.');}catch(e){setError(e.message);}finally{setBusy(false);}}}>Download approved packet · ZIP</button></details>}
  </aside><main>
