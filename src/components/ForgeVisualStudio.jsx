...
const stages=[['portrait','◉','Portrait'],['character','♟','Full character'],['master','◇','3D master'],['web','⬡','Web model']];
...
export default function ForgeVisualStudio({brief,onClose,onAdopt}){
 ...
 const [jobs,setJobs]=useState([]),[job,setJob]=useState(null),[operator,setOperator]=useState(false),[quotes,setQuotes]=useState({}),[busy,setBusy]=useState(false),[error,setError]=useState(''),[confirm,setConfirm]=useState(false),[view,setView]=useState('portrait'),[preview,setPreview]=useState(null),[viewed,setViewed]=useState(''),[loading,setLoading]=useState(false),[notice,setNotice]=useState('');
 ...
 const ready=useCallback(()=>setViewed(assetHash||''),[assetHash]);
 async function act(input){setBusy(true);setError('');try{const result=await visualRequest(input,true);if(input.action==='adopt'){setNotice('Approved portrait and 3D model are now used in Forge.');onAdopt();}else{setJob(result);setJobs(list=>[result,...list.filter(x=>x.id!==result.id)]);}setConfirm(false);}catch(e){setError(e.message);}finally{setBusy(false);}}
 ...
 const current=job?.stages[job.stage],canReview=job?.status==='review'&&view===job.stage&&viewed===assetHash;
 return <dialog ref={dialog} className="forge-visual-dialog" aria-labelledby="visual-title" onCancel={onClose}>
  ...
  <div className="visual-layout"><aside>
   <h2>Artist Packet Expert</h2><p className="visual-muted">Card → portrait → full character → 3D</p>
   ...
   <ol className="visual-stages">{stages.map(([key,icon,label])=><li key={key}><button className={view===key?'selected':''} disabled={!job?.assets[key]} onClick={()=>setView(key)}><span>{icon}</span><span>{label}<small>{job?.stages[key]?.status?.replaceAll('_',' ')||'Pending'}</small></span>{job?.stages[key]?.review?.decision==='approve'?'✓':''}</button></li>)}</ol>
   {job?.assets.card&&<button onClick={()=>setView('card')}>▧ Compare source artwork</button>}
   ...
   {job&&<section className="visual-decision"><div className="visual-status"><strong>{statusText[job.status]||job.status}</strong><span>{stages.find(s=>s[0]===job.stage)?.[2]}</span>{current?.progress!=null&&<progress max="100" value={current.progress}/>}</div>
    ...
    {job.status==='draft'&&(confirm?<div className="visual-consent"><h3>Approve this generation</h3><p>{stageCost[job.stage]}</p><p>One attempt. Your reference image is sent to {['portrait','character'].includes(job.stage)?'OpenAI':'Meshy'}. No paid automatic retries.</p><button className="visual-primary" disabled={busy||!operator} onClick={()=>act({action:'start',id:job.id,revision:job.revision,stage:job.stage,sha256:job.stage==='portrait'?job.brief_sha256:job.assets[{character:'portrait',master:'character',web:'master'}[job.stage]]?.sha256,quote:quotes[job.stage]})}>{busy?'Saving approval…':'Approve charge & generate'}</button><button disabled={busy} onClick={()=>setConfirm(false)}>Cancel</button></div>:<button className="visual-primary" disabled={busy||!operator} onClick={()=>setConfirm(true)}>Generate {stages.find(s=>s[0]===job.stage)?.[2].toLowerCase()} →</button>)}
    {job.status==='review'&&<><p>{canReview?'Check identity, palette, complete limbs and detail before continuing.':'Open and inspect this stage’s output before approving it.'}</p><div className="visual-actions"><button className="visual-primary" disabled={busy||!operator||!canReview} onClick={()=>act({action:'review',id:job.id,revision:job.revision,stage:job.stage,sha256:job.assets[job.stage].sha256,decision:'approve'})}>Approve & continue</button><button disabled={busy||!operator} onClick={()=>act({action:'review',id:job.id,revision:job.revision,stage:job.stage,sha256:job.assets[job.stage].sha256,decision:'reject'})}>Needs revision</button></div></>}
