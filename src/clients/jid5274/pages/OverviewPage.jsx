...
import { fetchJid5274Schedules, fetchJid5274Runs, fetchJid5274Board } from '../lib/portalData';
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([fetchJid5274Schedules(), fetchJid5274Runs(), fetchJid5274Board()])
      .then(([s, r, b]) => {
        if (cancelled) return;
        setSchedules(s || []);
        setRuns(r || []);
      ...
      .catch((err) => {
        if (!cancelled) setError(err.message || 'Unable to load jid5274 data');
      });
    return () => {
      cancelled = true;
    };
  ...
  const cards = [
    ...
    { label: 'Run cost', value: formatCost(totalCost), detail: 'Total spend across captured runs', icon: CircleDollarSign },
  ];
    ...
    >
      <div className="space-y-3">
        <CollapsibleSection
          eyebrow="01"
          title="Summary"
          meta={<span className="gb-pill">jid5274</span>}
          defaultOpen
        >
          {error && (
            <p className="rounded-md border border-red-200 bg-red-50 p-5 text-sm text-red-700">{error}</p>
          )}
          {!error && loading && <p className="text-sm text-[#191919]/60">Loading jid5274 telemetry…</p>}
          {!error && !loading && (
        ...
        >
          <div className="rounded-md border border-[#D6D4C8] bg-[#191919] p-5 text-[#F3F1E7]">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#F3F1E7]/70">
              ...
              jid5274 schedules. Empty states are expected while the tenant warms up.
            </p>
          </div>
        </CollapsibleSection>
