...
import OpsPageShell, { SectionHeader } from '../components/OpsPageShell';
...
/**
 * ViewShell — themed frame shared by every observability sub-view. Renders the
 * canonical SectionHeader (via OpsPageShell), the page intro, and the persistent
 * tab strip, then the per-view collapsible sections as children. Presentation
 * only — every sub-view keeps its own hooks/data wiring.
 */
function ViewShell({ number, title, action, intro, children }) {
  return (
    <OpsPageShell number={number} title={title} meta={action}>
      {intro ? (
        <p className="max-w-4xl text-sm leading-7 text-[#191919]/70">{intro}</p>
      ) : null}
      <ObservabilityNav />
...
function SummaryMetric({ label, value, detail, icon: MetricIcon }) {
  return (
    <article className="glass-panel hover-mini rounded-md p-5">
...
function Overview() {
  ...
  return (
    <ViewShell
      ...
      intro="Dedicated pages for Hermes, Claude Code, Codex, local session archives, and the ecom telemetry report."
    >
      <CollapsibleSection
        eyebrow="01"
        title="Summary Metrics"
        meta={<span className="gb-pill">{metrics.length} metrics</span>}
        defaultOpen
      >
      ...
      <CollapsibleSection
        ...
        meta={<span className="gb-chip gb-chip-amber">2 gaps</span>}
      >
      ...
      >
        <div className="rounded-md border border-[#D6D4C8] bg-[#191919] p-5 text-[#F3F1E7]">
          <p className="text-sm leading-6 text-[#F3F1E7]/75">
      ...
      <CollapsibleSection
        eyebrow="04"
        title="Sources"
        meta={<span className="gb-pill">{observabilityData.sources.length} sources</span>}
      >
...
function Traces() {
  ...
  return (
    <ViewShell
      number="Langfuse Logs"
      title="Trace Browser"
      intro="Server-side Langfuse reads use an authenticated Amplify query backed by a Lambda that reads AWS Secrets Manager; otherwise the page stays usable with the latest archived report data."
    >
      <CollapsibleSection
        eyebrow="01"
        title={
        ...
        meta={<span className="gb-chip gb-chip-blue">{harness}</span>}
      >
      ...
      <CollapsibleSection
        eyebrow="02"
        title="Traces"
        meta={<span className="gb-pill">{filtered.length} traces</span>}
        defaultOpen
      >
        <div className="overflow-hidden rounded-md border border-[#D6D4C8] bg-white/50">
          {filtered.map((trace) => (
      ...
      {selectedTrace ? (
        <CollapsibleSection
          eyebrow="03"
          title="Observation Tree"
          meta={<span className="gb-pill">{selectedObservations.length} obs</span>}
        >
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              <span className="gb-eyebrow text-[#D97757]">Observation Tree</span>
...
function Sessions() {
  return (
    <ViewShell
      number="Local Archives"
      title="Session Directory Map"
      intro="Hermes, Claude Code, and Codex archives are indexed here so local session files can be reconciled with Langfuse traces."
    >
      <CollapsibleSection
        eyebrow="01"
        title="Session Directory"
        meta={<span className="gb-pill">{observabilityData.sessions.length} sessions</span>}
        defaultOpen
      >
        <section className="grid gap-4 lg:grid-cols-2">
          {observabilityData.sessions.map((session) => (
            <article key={session.id} className="glass-panel hover-mini rounded-md p-5">
...
function Agents() {
  return (
    <ViewShell
      number="Profiles"
      title="Agent Emission Contracts"
      intro="Each assistant profile needs stable harness, profile, repo, slug, branch, model, and session metadata."
    >
      <CollapsibleSection
        eyebrow="01"
        title="Emission Contracts"
        meta={<span className="gb-pill">{observabilityData.agents.length} agents</span>}
        defaultOpen
      >
        <section className="grid gap-5 lg:grid-cols-3">
          {observabilityData.agents.map((agent) => (
            <article key={agent.name} className="glass-panel hover-mini rounded-md p-5">
              ...
              <div className="mt-5">
                <p className="gb-eyebrow">Emitted</p>
...
function Reports() {
  return (
    <ViewShell
      number="Reports"
      title="Observability Archive"
      intro="Durable report links copied from the gbautomation telemetry and Langfuse report artifacts."
    >
      <CollapsibleSection
        eyebrow="01"
        title="Report Archive"
        meta={<span className="gb-pill">{observabilityData.reports.length} reports</span>}
        defaultOpen
      >
        <div className="overflow-hidden rounded-md border border-[#D6D4C8] bg-white/50">
...
function Live() {
  ...
  return (
    <ViewShell
      number="Template Viewer"
      title="Live Event Timeline"
      intro="Adapted from the Pi Observability frontend: single-session timeline, swimlane view, and race view over the same ObsEvent-shaped rows."
    >
      <CollapsibleSection
        eyebrow="01"
        title="Live Timeline"
        meta={<span className="gb-chip gb-chip-green">{view}</span>}
        defaultOpen
      >
...
function InventoryFooter() {
  return (
    <div className="space-y-3">
      <SectionHeader number="REPO" title="Local Inventory" />
      <CollapsibleSection
        eyebrow="A"
        title={
          <span className="inline-flex items-center gap-2">
        ...
        meta={<span className="gb-pill">{observabilityData.committers.length}</span>}
      >
      ...
      <CollapsibleSection
        ...
        meta={<span className="gb-pill">{observabilityData.worktrees.length}</span>}
      >
        <div className="max-h-72 overflow-auto">
...
export default function OpsObservability() {
  ...
  return (
    <div className="space-y-10">
      {body}
      <InventoryFooter />
      <div className="flex flex-wrap items-center gap-3 border-t border-[#D6D4C8]/70 pt-6 text-xs text-[#191919]/45">
